...
.EXAMPLE
PS> .\Get-ImpervaPolicyByName.ps1 -apiId 40123 -apiKey 12345678-abcd-efgh-1234-1a2b3c4d5e6f -siteName 'Some Site'
...
param (
    ...
    $apiKey,
    [Parameter(Mandatory=$true,
    HelpMessage="Enter the Site Name value")]
    [string]
    $siteName
)
...
try {
    $response = Invoke-WebRequest -Uri https://my.imperva.com/api/prov/v1/sites/list -Method POST -Body $postParams -UseBasicParsing
    if ($response.statusCode -eq 200) {
        # Recieved a valid HTTP 200 response, so extract content
        $jsonData = $response.content
        # Convert to a useful format
        $data = (ConvertFrom-Json -InputObject $jsonData).sites
    }
}
catch {
...
# Return data
$data | Where-Object { $_.display_name -eq $siteName}
